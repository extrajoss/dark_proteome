...
var dark_proteome_postProcessing = require('./dark_proteome_postProcessing')

exports.check_progress = function () {
    return connector.queryPromise("SELECT greatest(" +
        "(SELECT count(DISTINCT accession_id)progress FROM non_dark_region)," +
        "(SELECT count(DISTINCT accession_id)progress FROM dark_region)" +
        ") progress;", [])
        .then(function (data) {
            return data;
        });
...
exports.get_dark_regions = function (request, response, next, options) {
    var rq = request;
...
var save_remaining_non_dark_regions = function () {
    var sqlQuery = {};
        ...
        .then(function (accessions) {
            ...
            return Promise.all(accessionPromises)
                .then(function (results) {
                    save_remaining_non_dark_regions();
                });
        }).catch(function (err) {
            console.log(err);
        });
...
var save_remaining_dark_regions = function () {
    var sqlQuery = {};
    sqlQuery.sql = "SELECT Id, Primary_Accession FROM accession WHERE Id NOT IN (SELECT Accession_Id FROM dark_region) AND Id NOT IN (SELECT Accession_Id FROM non_dark_region) limit 500";
    sqlQuery.args = [10];
    return connector.queryPromise(sqlQuery.sql, sqlQuery.args)
        .then(function (accessions) {
            ...
            if (accessionPromises.length == 0) {
                return dark_proteome_postProcessing.start_post_processing();
            }
            return Promise.all(accessionPromises)
                .then(function (results) {
...
var get_dark_regions_from_accession = function (uniprot_primary_accession, options, finalCallback) {
    var accession = { "Primary_Accession": uniprot_primary_accession };
    get_dark_regions(accession, options).then(function (results) {
        finalCallback(results.accession, results.regions);
    });
}

var get_dark_regions = function (accession, options) {
    return get_regions(accession, true, options)
}
...
var get_regions = function (accession, calculate_dark, options) {
    ...
    var result = get_protein_sequence_sql(uniprot_primary_accession)
        ...
        function (sqlQuery) {
            ...
            return connector.queryPromise(sqlQuery.sql, sqlQuery.args, options);
        }).then(function (results) {
            ...
            if (calculate_dark) {
                return generate_dark_regions(uniprot_primary_accession, matches.sequences[0], options);
            } else {
                return generate_non_dark_regions(uniprot_primary_accession, matches.sequences[0], options);
            }
...
var get_protein_sequence_sql = function (primaryAccession) {
    return new Promise(
        function (resolve) {
            ...
            FROM aquaria.protein_sequence WHERE Primary_Accession = ?";
...
function generate_non_dark_regions(uniprot_primary_accession, sequence, options) {
    'use strict';
...
function generate_dark_regions(uniprot_primary_accession, sequence, options) {
    'use strict';
...
var getUniprot_PDB = function (uniprot_primary_accession) {
    return new Promise(
        function (resolve) {
            ...
            FROM aquaria.Uniprot_PDB WHERE Primary_Accession = ?";
...
exports.save_remaining_non_dark_regions = save_remaining_non_dark_regions;
